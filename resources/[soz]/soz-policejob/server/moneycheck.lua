QBCore.Functions.CreateCallback("police:getOtherPlayerData", function(source, cb, target)
    ...
    if Target then
        TriggerClientEvent("hud:client:DrawNotification", target,
                           "Vous êtes en train d'être fouillé par le " .. srcJob, "info")
    ...
    for _, allowedJob in ipairs(Config.AllowedJobInteraction) do
        if Player.PlayerData.job.id == allowedJob then
            ...
            if Target.Functions.RemoveMoney("marked_money", markedAmount) and
                exports["soz-core"]:AddMoneyToSafeStorage("safe_" .. Player.PlayerData.job.id, markedAmount,
                                                          "marked_money", true) then
                ...
                TriggerClientEvent("hud:client:DrawNotification", Player.PlayerData.source, "Problème de confiscation",
                                   "error")
            end
